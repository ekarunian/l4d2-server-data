...
new Handle:sm_satellite_bileeffect		= INVALID_HANDLE;
...
public OnPluginStart()
{
	...
	sm_satellite_damage_01		= CreateConVar("sm_satellite_damage_01","320.0", "Damage of Satellite cannon (Mode:JUDGEMENT)", FCVAR_NOTIFY);
	...
	sm_satellite_damage_03		= CreateConVar("sm_satellite_damage_03","800.0", "Damage of Satellite cannon (Mode:INFERNO)", FCVAR_NOTIFY);
	sm_satellite_burst_delay	= CreateConVar("sm_satellite_burst_delay","2.0", "Launching delay of Satellite cannon", FCVAR_NOTIFY);
	...
	sm_satellite_limit_01		= CreateConVar("sm_satellite_limit_01","7", "Frequency limitation in one round (Mode:JUDGEMENT)", FCVAR_NOTIFY);
	sm_satellite_limit_02		= CreateConVar("sm_satellite_limit_02","5", "Frequency limitation in one round (Mode:BLIZZARD)", FCVAR_NOTIFY);
	sm_satellite_limit_03		= CreateConVar("sm_satellite_limit_03","5", "Frequency limitation in one round (Mode:INFERNO)", FCVAR_NOTIFY);
	sm_satellite_height			= CreateConVar("sm_satellite_height","600", "Height of launching point", FCVAR_NOTIFY);
	sm_satellite_adminonly		= CreateConVar("sm_satellite_adminonly","0", "Admin only (0:OFF 1:ON 2:INFINITY)", FCVAR_NOTIFY);
	sm_satellite_bileeffect		= CreateConVar("sm_satellite_bileeffect","1", "Boomer bile effect (0:OFF 1:ON)", FCVAR_NOTIFY);
...
public Action:Event_Weapon_Fire(Handle:event, const String:name[], bool:dontBroadcast)
{
		...
		GetConVarInt(sm_satellite_enable) && operation[attacker] > 0)
	{
		...
		/* Create chase point */
		if(GetConVarInt(sm_satellite_bileeffect))
			CreateChasePoint(attacker);
...
public CreateRingEffect(client, colRed, colGre, colBlu, alpha, Float:width, Float:duration)
{
	...
	TE_SetupBeamRingPoint(trsPos[client], 300.0, 10.0, g_BeamSprite,
						g_HaloSprite, 0, 10, GetConVarFloat(sm_satellite_burst_delay),
						4.0, 0.5, {150, 150, 230, 230}, 80, 0);
	TE_SendToAll();
}
...
public CreateChasePoint(client)
{
	decl entity;
	entity = CreateEntityByName("info_goal_infected_chase");
	DispatchKeyValueVector(entity, "origin", trsPos[client]);
	DispatchSpawn(entity);
	AcceptEntityInput(entity, "Enable");
	CreateTimer(GetConVarFloat(sm_satellite_burst_delay), DeleteEntity, entity);
}
